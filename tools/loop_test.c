...
int main(int argc, char *argv[])
{
    ...
    for(i = 0; i < num_loops; i++)
	{
		...
		if(size_read != size_written)
		{
			...
			printf("Size mismatch! Wrote %d bytes, read %d bytes.\n", size_written, size_read);
			total_failures += 1;
			continue;
		}
		...
		for(j = 0; j < size_read; j++)
			if(idata[j] != odata[j])
			{
				...
				printf("Loop %d: Data mismatch - tx_data[%4.4d]: 0x%2.2x != rx_data[%4.4d]: 0x%2.2x\n", i, j, odata[j], j, idata[j]); 
				failures++;
				if(failures > 4) break;
			}
